...
// ======================
// Plano da Noite (Accordion + Features Interativas)
// ======================
document.addEventListener('DOMContentLoaded', function() {
    // Só executa se a secção existir
    const planContainer = document.querySelector('.val-plan');
    if (!planContainer) return;

    /* --- Accordion Logic --- */
    const buttons = planContainer.querySelectorAll('.val-btn, .val-btn-final');

    buttons.forEach(button => {
        button.addEventListener('click', function() {
            const currentContent = this.nextElementSibling;
            const isActive = this.classList.contains('val-active');

            // Fechar todos
            planContainer.querySelectorAll('.val-btn, .val-btn-final').forEach(btn => {
                btn.classList.remove('val-active');
                btn.nextElementSibling.style.maxHeight = null;
            });

            // Abrir atual se não estava aberto
            if (!isActive) {
                this.classList.add('val-active');
                
                // Recalcular altura dinamicamente (para acomodar checklists que crescem)
                // Usamos setTimeout para garantir que elementos internos renderizaram
                setTimeout(() => {
    ...
    /* --- 1. Checklist Interativa --- */
    const checklistItems = [
        "Fazer uma massagem de 1 minuto miando como um gato",
        "Lamber a ponta do nariz do outro do nada",
        "Dançar a Macarena a meio de uma conversa séria",
        "Fazer um elogio apaixonado usando voz de robô",
        "Morder (devagarinho) a orelha e fugir a rir",
        "Tentar adivinhar a cor da roupa interior sem espreitar",
        "Desenhar um coração na perna do outro com o dedo",
        "Falar apenas a cantar (estilo ópera) durante 2 minutos",
        "Encenar uma discussão de novela mexicana dramática",
        "Fazer ataque de cócegas surpresa",
        "Dar 3 beijos de esquimó (nariz com nariz)",
        "Imitar um animal sexy (à escolha)",
        "Sussurrar uma receita de culinária no ouvido de forma sensual",
        "Fingir que és um T-Rex a tentar dar um abraço",
        "Elogiar o dedo mindinho do pé do outro com máxima seriedade",
        "Fazer uma careta horrível e aguentar o riso por 10 segundos",
        "Pedir o outro em casamento com um anel invisível (super dramático)",
        "Tentar lamber o cotovelo do outro (missão impossível)",
        "Mostrar partes íntimas num momento aleatório",
        "Fazer ou dizer algo aleatório durante o sexo"
    ];

    const checklistContainer = document.getElementById('val-checklist');
    const completedCountEl = document.getElementById('val-completed-count');
    ...
    if (checklistContainer) {
        ...
        const savedState = JSON.parse(localStorage.getItem('valChecklistState')) || {};
        ...
        checklistItems.forEach((item, index) => {
            ...
            div.innerHTML = `
                <label class="${isChecked ? 'completed' : ''}">
        ...
        checklistContainer.addEventListener('change', (e) => {
            if (e.target.tagName === 'INPUT') {
                ...
                localStorage.setItem('valChecklistState', JSON.stringify(savedState));
                completedCountEl.textContent = Object.keys(savedState).length;
            }
        });
    }

    /* --- 2. Taça do Destino (Gerador de Missões) --- */
    const missions = [
        "Dar 10 beijos rápidos como um pica-pau",
        "Abraço de urso mudo (30 segundos sem falar)",
        "Dançar uma música lenta imaginária agora mesmo",
        "Fazer um elogio muito específico sobre o nariz do outro",
        "Beijo de cinema (com inclinação e tudo)",
        "Olhar nos olhos e tentar não rir durante 1 minuto",
        "Fazer uma declaração de amor como se estivesses a morrer",
        "Recriar o primeiro beijo (com exagero)",
        "Imitar a gargalhada do outro",
        "Cozinhar (ou preparar um snack) vendado por 2 minutos",
        "Alimentar o outro com uma fruta de forma sedutora (e falhar)",
        "Dizer o alfabeto a dar um beijo a cada letra",
        "Massagem nos pés obrigatória de 3 minutos",
        "Tirar uma selfie a fazer a cara mais feia possível",
        "Dançar o 'Vira' ou algo folclórico agarradinhos",
        "Sussurrar um segredo parvo ao ouvido",
        "Fazer um brinde solene à nossa 'estranheza'",
        "Escrever 'Amo-te' nas costas do outro com o dedo"
    ];
    
    const missionDisplay = document.getElementById('val-mission-display');
    const missionBtn = document.getElementById('val-generate-mission');

    if (missionBtn && missionDisplay) {
        missionBtn.addEventListener('click', () => {
            // Efeito visual simples de "shuffle"
            missionDisplay.textContent = "...";
            let count = 0;
            const interval = setInterval(() => {
                missionDisplay.textContent = missions[Math.floor(Math.random() * missions.length)];
                count++;
                if (count > 5) {
                    clearInterval(interval);
                    // Escolha final
                    const finalMission = missions[Math.floor(Math.random() * missions.length)];
                    missionDisplay.textContent = finalMission;
                }
            }, 100);
        });
    }

    /* --- 3. Arte do Caos (Timer) --- */
    const timerDisplay = document.getElementById('val-timer-display');
    const timerBtn = document.getElementById('val-start-timer');
    let timerInterval;
    ...
    if (timerBtn && timerDisplay) {
        timerBtn.addEventListener('click', () => {
            ...
            timerInterval = setInterval(() => {
                ...
                if (timeLeft <= 0) {
                    ...
                    timerDisplay.textContent = "Tempo Terminado!";
                    alert("Tempo terminado! Mostrem as vossas obras-primas! 😂");
                    timerBtn.textContent = "Reiniciar Temporizador";
     ...
     /* --- 4. Cozinha do Caos (Regras) --- */
     const chaosRules = [
        "Cozinhar com uma mão",
        "Só comunicar por mímica",
        "Dançar enquanto cozinham",
        "Beijo antes de usar utensílio",
        "Trocar funções",
        "Cozinhar em câmara lenta",
        "Falar com sotaque inventado"
    ];
    
    const ruleDisplay = document.getElementById('val-rule-display');
    const ruleBtn = document.getElementById('val-new-rule');

    if (ruleBtn && ruleDisplay) {
    ...
    /* --- 10. Última Peça (Scoreboard) --- */
    // Função global para ser chamada pelo onclick no HTML
    window.updateValScore = function(person, change) {
        const el = document.getElementById(`val-score-${person}`);
        if (el) {
            let current = parseInt(el.textContent) || 0;
            current += change;
            if (current < 0) current = 0; // Sem negativos
